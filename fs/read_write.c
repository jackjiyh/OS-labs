...
testfs_allocate_block(struct inode *in, int log_block_nr, char *block)
{
    ...
    if (log_block_nr < NR_INDIRECT_BLOCKS) {
        ...
        if (dind[dind_nr] == 0) {
            ...
            if (phy_block_nr < 0)
                return phy_block_nr;
            dind_ind_alloc = 1;
            dind[dind_nr] = phy_block_nr;
        } else {
            read_blocks(in->sb, indirect, dind[dind_nr], 1);
        }
        /* allocate direct block */
    ...
    if (phy_block_nr >= 0) {
        ...
        if (!flag) {
            ...
            write_blocks(in->sb, indirect, dind[dind_nr], 1);
        }
    } else {
...
testfs_free_blocks(struct inode *in)
{
    ...
    if (e_block_nr >= 0) {
        ...
        for (i=0; i<dind_nr; i++) {
            read_blocks(in->sb, block, dind_block[i], 1);
            long temp = (i == dind_nr) ? ind_nr : BLOCK_SIZE;
            for (j=0; j < temp; j++) {
